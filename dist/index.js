...
import { DEFAULT_IMAGE_MODEL, generateAntigravityImage, IMAGE_ASPECT_RATIOS, parseImageCommandArgs, } from "./image/index.js";
...
import { fetchAccountUsage, formatModelsList, formatUsageSummary, resolveApiKeyFromContext, } from "./usage/index.js";
 ...
 */
function emitCommandOutput(ctx, text, type = "info") {
    if (ctx.hasUI) {
...
async function withUsage(ctx, fn) {
    try {
        const apiKey = await resolveApiKeyFromContext(ctx);
        if (!apiKey) {
            emitCommandOutput(ctx, "No Antigravity credentials. Run /login antigravity first.", "warning");
            return;
        }
        if (ctx.hasUI)
            ctx.ui.notify("Fetching Antigravity usage…", "info");
...
export default function (pi) {
    ...
    pi.registerCommand("antigravity.image", {
        description: "Generate an image via Antigravity (usage: /antigravity.image [--ratio 16:9] <prompt>)",
        handler: async (args, ctx) => {
            const parsed = parseImageCommandArgs(args || "");
            if (!parsed.prompt) {
                emitCommandOutput(ctx, "Usage: /antigravity.image [--ratio 16:9] [--model gemini-3-pro-image] [--path file.png] <prompt>", "warning");
                return;
            }
            try {
                const apiKey = await resolveApiKeyFromContext(ctx);
                ...
                if (ctx.hasUI)
                    ctx.ui.notify("Generating Antigravity image…", "info");
