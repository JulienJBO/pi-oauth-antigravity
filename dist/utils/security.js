...
export function resolveCallbackHost(raw = antigravityEnv("CALLBACK_HOST")) {
    const host = (raw || "127.0.0.1").trim().toLowerCase();
    if (!LOOPBACK_HOSTS.has(host)) {
        throw new Error(`Unsafe ANTIGRAVITY_CALLBACK_HOST="${host}". Only loopback hosts are allowed: 127.0.0.1, ::1, localhost.`);
    }
    return host === "localhost" ? "127.0.0.1" : host;
}
/** Prevent token exfiltration via poisoned BASE_URL (SSRF / credential leak). */
export function assertSafeApiBaseUrl(raw) {
    let url;
    try {
        url = new URL(raw);
    }
...
export function redactSecrets(text) {
    return text
        ...
        // Google refresh tokens mint as "1//04…" (double slash); match both spellings.
        .replace(/\b1\/{1,2}[A-Za-z0-9_-]{20,}/g, "[redacted-refresh-token]")
        .replace(/\bBearer\s+[A-Za-z0-9._~+/-]+=*/gi, "Bearer [redacted]")
        .replace(/("?(?:access_token|refresh_token|id_token|token|client_secret|code_verifier|authorization)"?\s*[:=]\s*")[^"]*(")/gi, "$1[redacted]$2")
        .replace(/("?(?:access_token|refresh_token|id_token|token|client_secret|code_verifier|authorization)"?\s*[:=]\s*)[^\s&,}]+/gi, "$1[redacted]");
}
