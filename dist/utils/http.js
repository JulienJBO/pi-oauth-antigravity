...
function hasProxyConfiguration() {
    return ["HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY", "http_proxy", "https_proxy", "all_proxy"].some((name) => Boolean(process.env[name]?.trim()));
}
...
export async function antigravityFetch(input, init = {}) {
 ...
 */
export function prewarmConnection(url) {
    if (antigravityEnv("NO_PREWARM") === "1")
        return;
    void (async () => {
        try {
