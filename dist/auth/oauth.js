...
export const CLIENT_SECRET = antigravityEnv("CLIENT_SECRET") || atob("R09DU1BYLUs1OEZXUjQ" + "4NkxkTEoxbUxCOHNYQzR6NnFEQWY=");
...
function sanitizeOAuthProviderError(text) {
    ...
    try {
        ...
        const parts = [parsed.error, parsed.error_description].filter((part) => typeof part === "string" && part.length > 0);
        if (parts.length)
    ...
    return new Promise((resolve, reject) => {
        ...
        server.on("error", (err) => {
            if (err.code === "EADDRINUSE") {
                reject(new Error("Port 51121 is already in use by another process. Please close the process using port 51121 and retry /login antigravity."));
            }
            else {
                reject(err);
            }
...
function parsePastedCallback(raw, expectedState) {
    const text = (raw ?? "").trim();
    if (!text) {
        throw new Error("No callback pasted. Paste the full URL from your browser's address bar (http://localhost:51121/oauth-callback?…).");
    }
    let url;
    try {
        url = new URL(text);
    ...
    const error = url.searchParams.get("error");
    if (error)
        throw new Error(`OAuth error from browser: ${error.slice(0, 200)}`);
    const code = url.searchParams.get("code");
    const state = url.searchParams.get("state");
    if (!code || !state) {
        throw new Error("Pasted text is missing 'code' or 'state'. Paste the FULL callback URL from your browser's address bar (http://localhost:51121/oauth-callback?…).");
    }
    if (state !== expectedState) {
        throw new Error("State mismatch: that callback is from a different sign-in. Re-run /login antigravity, sign in again, and paste the new URL.");
    }
    return { code, state };
}
/**
...
async function acquireAuthCode(expectedState, opts) {
    ...
    if (typeof callbacks.onPrompt === "function") {
        const promptLoop = async () => {
            let promptMessage = "Remote/headless machine (your browser can't reach localhost:51121)? " +
    ...
    if (callbacks.signal) {
        candidates.push(new Promise((_resolve, reject) => {
...
export async function loginAntigravity(callbacks) {
...
export async function refreshAntigravityToken(credentials, signal) {
    ...
    if (!response.ok) {
        throw new Error(`Antigravity token refresh failed: ${sanitizeOAuthProviderError(await response.text())}`);
    }
