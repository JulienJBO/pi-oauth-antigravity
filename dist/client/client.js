...
import { getCurrentAvailableModels, getCurrentEndpoint, getCurrentMatchedModelDebug, setLastAvailableModels, setLastEndpoint, setLastError, setLastMatchedModelDebug, setLastStatus, } from "../diagnostics/diagnostics.js";
...
export function endpointCandidates(preferredEndpoint) {
...
export function parseApiKey(apiKeyRaw) {
    if (!apiKeyRaw) {
...
export function extractProjectId(data) {
    if (!isRecord(data))
        return undefined;
    const direct = data.antigravityProjectId ??
        data.projectId ??
...
function collectModelLabels(value, out = []) {
    if (!value || out.length > 50)
        return out;
    if (typeof value === "string") {
        if (/gemini|claude|gpt-oss/i.test(value))
            out.push(value);
        return out;
    }
    if (Array.isArray(value)) {
        for (const item of value)
            collectModelLabels(item, out);
        return out;
    }
    if (isRecord(value)) {
...
function summarizeModelCandidate(value) {
    ...
    for (const [key, raw] of Object.entries(value)) {
        if (/token|auth|credential|secret|email/i.test(key))
            continue;
        if (raw === null || ["string", "number", "boolean"].includes(typeof raw))
            out[key] = raw;
        else if (Array.isArray(raw))
            out[key] = `[array:${String(raw.length)}]`;
...
export function isUsableRuntimeModelId(id) {
...
function buildModelMatchRegex(requestedId) {
    ...
    if (req === "gemini-3.8-flash-low")
        return /gemini[- ]3\.8[- ]flash \(low\)/i;
    if (req === "gemini-3.8-flash-medium")
        return /gemini[- ]3\.8[- ]flash \(medium\)/i;
    if (req === "gemini-3.8-flash-high")
        return /gemini[- ]3\.8[- ]flash \(high\)/i;
    if (req === "gemini-3.7-flash-low")
        return /gemini[- ]3\.7[- ]flash \(low\)/i;
    if (req === "gemini-3.7-flash-medium")
        return /gemini[- ]3\.7[- ]flash \(medium\)/i;
    if (req === "gemini-3.7-flash-high")
        return /gemini[- ]3\.7[- ]flash \(high\)/i;
    if (req === "gemini-3.6-flash-low")
        return /gemini[- ]3\.6[- ]flash \(low\)/i;
    if (req === "gemini-3.6-flash-medium")
        return /gemini[- ]3\.6[- ]flash \(medium\)/i;
    if (req === "gemini-3.6-flash-high")
        return /gemini[- ]3\.6[- ]flash \(high\)/i;
    if (req === "gemini-3.5-flash-extra-low")
        return /gemini[- ]3\.5[- ]flash \(low\)/i;
    ...
    if (req.includes("claude-opus-4-6"))
        return /claude.*opus.*4\.6/i;
    if (req.includes("claude-sonnet-4-6"))
        return /claude.*sonnet.*4\.6/i;
    if (req.includes("gpt-oss-120b"))
        return /gpt.*oss.*120b/i;
    if (req === "gemini-3.1-pro-low")
        return /gemini[- ]3\.1[- ]pro \(low\)/i;
...
function findDynamicModel(value, requestedId) {
    ...
    if (isRecord(value) && isRecord(value.models)) {
        ...
        for (const [modelId, info] of Object.entries(modelsMap)) {
            if (!isUsableRuntimeModelId(modelId))
                continue;
            if (targetRegex.test(modelId))
...
export async function fetchAvailableRuntimeModel(token, projectId, requestedRuntimeModel) {
    ...
    if (inFlight)
        return inFlight;
    const promise = fetchAvailableRuntimeModelUncached(token, projectId, requestedRuntimeModel).then((result) => {
...
export function resolveProjectId(opts) {
    return (antigravityEnv("PROJECT_ID")?.trim() ||
        opts.warmedProject ||
