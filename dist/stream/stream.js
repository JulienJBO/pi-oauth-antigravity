...
import { antigravityHeaders, endpointCandidates, fetchAvailableRuntimeModel, formatRequestDiagnostics, jsonOrTextError, loadCodeAssist, parseApiKey, resolveProjectId, } from "../client/client.js";
import { getCurrentEndpoint, runWithDiagnostics, setLastEndpoint, setLastError, setLastLatencyMs, setLastProjectId, setLastResolvedRuntimeModel, setLastStatus, } from "../diagnostics/diagnostics.js";
import { AntigravityRequestType, AntigravityUserAgent, GeminiRole, GeminiToolCallingMode, StopReason, ToolChoice, } from "../types/enums.js";
...
const ANTIGRAVITY_NO_PREAMBLE_INSTRUCTION = 'CRITICAL: NEVER output rule checks, formatting guidelines, constraint checklists (e.g. "No emdashes"), or your thinking/personality preambles in the final response. Output only the final response.';
...
export function isPlanQuotaError(status, text) {
...
function quoteBackendMessage(text) {
...
function asTextParts(content) {
    if (typeof content === "string")
        return [{ text: sanitizeText(content) }];
...
export function convertMessages(model, context, runtimeModel) {
    const contents = [];
    ...
    for (const msg of context.messages) {
        if (msg.role === "user") {
            ...
            const allSigsValid = toolCalls.every((tc) => !tc.thoughtSignature || isValidThoughtSignature(tc.thoughtSignature));
            const groupIsSigned = isSameModel && firstCallHasSig && allSigsValid;
            for (const block of msg.content) {
                if (block.type === "text") {
                    const textSig = isSameModel && isValidThoughtSignature(block.textSignature)
                        ? block.textSignature
                        : undefined;
...
function normalizeCustomToolType(value) {
    ...
    const entries = value;
    const scalar = entries.find((entry) => typeof entry === "string" && entry !== "null");
    return scalar;
}
function normalizeCustomToolSchema(schema) {
    if (!schema || typeof schema !== "object")
        return schema;
    if (Array.isArray(schema))
        return schema.map(normalizeCustomToolSchema);
    const out = {};
    for (const [key, value] of Object.entries(schema)) {
 ...
 */
export function convertTools(tools, useLegacyParameters = false, supportsStrictMode = false) {
    if (!tools?.length)
        return undefined;
    return [
        {
            functionDeclarations: tools.map((tool) => {
                const parameters = getJsonSchemaToolParameters(tool, resolveJsonSchemaStrictSampling(tool, supportsStrictMode));
...
function mapToolChoiceMode(toolChoice) {
    if (toolChoice === ToolChoice.None)
        return GeminiToolCallingMode.None;
...
/** Exported for unit tests. */
export function buildRequest(model, context, projectId, options, runtimeModel, sessionState) {
    ...
    const tools = convertTools(getCurrentTools(context), isClaude || model.id.startsWith("gpt-oss-"), 
    ...
    if (tools) {
        ...
        request.toolConfig = {
            functionCallingConfig: {
                mode: options.toolChoice && options.toolChoice !== ToolChoice.Auto
...
/** Exported for unit tests. */
export async function streamResponse(response, stream, output, model, sessionState) {
    if (!response.body)
...
export function streamAntigravity(model, context, options) {
            ...
            for (let emptyAttempt = 0; emptyAttempt <= 2; emptyAttempt++) {
                ...
                for (let candIdx = 0; candIdx < runtimeCandidates.length; candIdx++) {
                    ...
                    for (const endpoint of endpointCandidates(sessionState.lastGoodEndpoint)) {
                        ...
                        for (;;) {
                            response = await antigravityFetch(`${endpoint}/v1internal:streamGenerateContent?alt=sse`, {
                                method: "POST",
                        ...
                        if (response.status === 429)
                            break;
                        if (![403, 404, 500, 502, 503, 504].includes(response.status))
                            break;
                    }
                    if (response?.ok)
                        break;
                ...
                if (!response || !response.ok) {
                    if (antigravityEnv("DEBUG_DUMP") === "1") {
                        try {
                            ...
                            await (await import("node:fs/promises")).writeFile("/tmp/antigravity-last-request.json", JSON.stringify({
                    ...
                    throw new Error(`Antigravity API error (${response?.status ?? "no response"}, ${formatRequestDiagnostics({ projectId, runtimeModel })}): ${friendly}`);
                }
                output.content = [];
                output.usage = {
                    input: 0,
