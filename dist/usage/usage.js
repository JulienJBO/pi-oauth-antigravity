import { antigravityHeaders, endpointCandidates, extractProjectId, parseApiKey, resolveProjectId, } from "../client/client.js";
import { setLastEndpoint, setLastError, setLastProjectId, setLastStatus, } from "../diagnostics/diagnostics.js";
...
async function postJson(path, token, body) {
    ...
    for (const endpoint of endpointCandidates()) {
        try {
            ...
            if (!res.ok) {
                const errorBody = isRecord(data) ? data : undefined;
                lastErrorText =
...
async function fetchAvailableModelsFromEndpoint(endpoint, token, projectId) {
    ...
    try {
        ...
        if (!res.ok) {
            const errorBody = isRecord(data) ? data : undefined;
            const lastErrorText = typeof errorBody?.error?.message === "string" ? errorBody.error.message : text;
            setLastError(lastErrorText);
            return undefined;
        }
        return { endpoint, status: res.status, data };
...
async function fetchMergedAvailableModels(token, projectId) {
    ...
    const results = await Promise.all(endpointCandidates().map((endpoint) => fetchAvailableModelsFromEndpoint(endpoint, token, projectId)));
    const mergedModels = {};
...
function parseQuotaSummary(data) {
    ...
    for (const group of summary.groups || []) {
        ...
        for (const bucket of group.buckets || []) {
            ...
            if (remaining === undefined && !bucket.bucketId)
                continue;
            buckets.push({
        ...
        if (!buckets.length && !group.displayName)
            continue;
        groups.push({
...
function parseModels(data) {
    ...
    for (const [modelId, info] of Object.entries(modelsObj)) {
        if (!info || !isRecord(info))
            continue;
        if (info.isInternal || String(modelId).startsWith("chat_"))
            continue;
        const qi = isRecord(info.quotaInfo) ? info.quotaInfo : {};
        models.push({
            modelId,
            displayName: typeof info.displayName === "string"
                ? info.displayName
            ...
            modelProvider: typeof info.modelProvider === "string"
                ? info.modelProvider
    ...
    return {
        models,
        defaultAgentModelId: raw.defaultAgentModelId || raw.defaultAgentModel
...
async function loadCodeAssistSafe(token) {
    try {
 ...
 */
async function fetchQuotaSummarySafe(token) {
    try {
...
export async function fetchAccountUsage(apiKeyRaw) {
    const creds = parseApiKey(apiKeyRaw);
    const initialProjectId = creds.projectId ||
        resolveProjectId({
            token: creds.token,
...
export function formatUsageSummary(usage) {
    ...
    for (const group of usage.groups) {
        ...
        for (const bucket of group.buckets) {
            ...
            lines.push(`  ${progressBar(bucket.remainingFraction)} ${bucket.displayName}: ${rem ?? "?"}% left · resets ${formatReset(bucket.resetTime)}`);
        }
    }
    return lines.join("\n").trimEnd();
}
export function formatModelsList(usage, opts) {
    const lines = [];
    lines.push("Antigravity available models");
    lines.push(`project=${usage.projectId}`);
    if (usage.defaultAgentModelId)
        lines.push(`defaultAgentModel=${usage.defaultAgentModelId}`);
    lines.push("");
    const rows = opts?.all
        ? usage.models
    ...
    for (const m of rows) {
        ...
        lines.push(`${m.modelId.padEnd(maxId)}  rem ${rem === undefined ? "  ?" : String(rem).padStart(5)}%  reset ${formatReset(m.resetTime).padEnd(8)}${flags ? `  [${flags}]` : ""}${name}`);
    }
    lines.push("");
    lines.push("Note: remaining % is pool-shared (not a private per-model budget).");
    return lines.join("\n");
}
export async function resolveApiKeyFromContext(ctx) {
    try {
